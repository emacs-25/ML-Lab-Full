...
#+PROPERTY: header-args:python :session har :exports both :results output

# NOTE for Emacs config: add (setq org-latex-listings t) to your init file so
# code blocks export via \lstlisting (respecting the breaklines=true set above)
# instead of a plain verbatim environment.
...
*Input:* 561 time- and frequency-domain features extracted from smartphone
accelerometer and gyroscope signals (50 Hz, windowed into 2.56s segments)
for 30 volunteers performing 6 activities.

*Output:* Cluster assignments produced independently by K-Means, DBSCAN, and
Hierarchical (Ward) clustering, with no access to the true activity label
during clustering.

*Objective:* Since clustering is unsupervised, the true activity labels
...
#+NAME: fig-class-distribution
#+CAPTION: Class distribution of the six activities
...
*Inference:* The class distribution is close to balanced across the six
activities (1406-1944 samples each), so no resampling is needed before
clustering.

#+NAME: fig-correlation-matrix
#+CAPTION: Correlation matrix of the first 40 features
#+ATTR_LATEX: :width 0.7\linewidth :placement [H]
#+begin_src python :results file
...
*Inference:* Large contiguous blocks of highly correlated features appear,
...
#+NAME: fig-feature-distributions
#+CAPTION: Distributions of four representative features, split by activity
...
*Inference:* Static activities (SITTING, STANDING, LAYING) separate clearly
...
- Encoding: activity labels are integer-coded 1-6 in the raw files; mapped
  to their names via =activity_labels.txt=, and kept aside only for
  post-hoc external validation.
- Feature scaling: =StandardScaler= (zero mean, unit variance) applied to
  all 561 features.
- Train-test split: not applicable in the supervised sense -- the original
  split is recombined into one set for clustering.
- Feature engineering: PCA is applied after scaling, to reduce
  dimensionality for clustering and to produce 2D projections for
  visualization.

#+NAME: fig-pca-variance
...
#+CAPTION: Ground-truth activities in PCA(2D) and t-SNE(2D)
...
** K-Means Clustering
- Working principle: partitions data into $k$ clusters by iteratively
...
- Mathematical formulation: minimizes
  $\text{WCSS} = \sum_{j=1}^{k}\sum_{x_i \in C_j} \lVert x_i - \mu_j \rVert^2$.
- Advantages: simple, fast, scales well; guaranteed to converge.
- Limitations: requires $k$ in advance; assumes roughly spherical,
  similarly-sized clusters; sensitive to initialization and feature scale.

** DBSCAN
- Working principle: groups points that are densely packed (density-
  connected within radius =eps=, with at least =minPts= neighbors), and
  marks points in low-density regions as noise.
- Mathematical formulation: a point $p$ is a *core point* if
  $|N_{eps}(p)| \geq \text{minPts}$; clusters are maximal sets of
  density-connected points.
- Advantages: no need to fix $k$ in advance; finds arbitrarily-shaped
  clusters; naturally identifies outliers.
- Limitations: sensitive to =eps=/=minPts=; struggles in high-dimensional
...
- Working principle: starts with every point as its own cluster and
...
- Mathematical formulation: Ward's criterion merges clusters $A, B$
  minimizing $\Delta(A,B) = \frac{|A||B|}{|A|+|B|}\lVert \mu_A - \mu_B \rVert^2$.
- Advantages: no need to pre-specify $k$; produces a full hierarchy;
  deterministic.
- Limitations: $O(n^2)$ or worse time/memory; greedy merges cannot be
...
*Selected k:* internal metrics point to $k=2$ (moving vs. not moving).
...
** DBSCAN -- eps and minPts Tuning

#+NAME: fig-dbscan-kdist
#+CAPTION: k-distance graph used to select DBSCAN's eps
#+ATTR_LATEX: :width 0.7\linewidth :placement [H]
#+begin_src python :results file
minPts = 10
...
*Chosen parameters: eps = 1.0, minPts = 10* -- the smallest eps giving
...
#+NAME: fig-dendrogram
#+CAPTION: Dendrogram (Ward linkage, truncated to last 30 merges)
#+ATTR_LATEX: :width 0.9\linewidth :placement [H]
#+begin_src python :results file
...
#+NAME: fig-kmeans-clusters
#+CAPTION: K-Means (k=6) clusters in PCA(2D) and t-SNE(2D)
#+ATTR_LATEX: :width 0.9\linewidth :placement [H]
#+begin_src python :results file
fig, axes = plt.subplots(1, 2, figsize=(13, 5.5))
...
*Inference:* K-Means (k=6) cleanly isolates LAYING but splits the dynamic-
activity region into clusters that don't map 1:1 onto the WALKING
variants, and merges SITTING with STANDING.

#+NAME: fig-dbscan-clusters
#+CAPTION: DBSCAN (eps=1.0, minPts=10) clusters in PCA(2D)
...
*Inference:* DBSCAN finds two large, well-separated density regions
(roughly dynamic vs. static) plus a few small tight sub-clusters and a
thin scatter of noise along the boundary.

#+NAME: fig-hac-clusters
#+CAPTION: Hierarchical (Ward, k=6) clusters in PCA(2D) and t-SNE(2D)
#+ATTR_LATEX: :width 0.9\linewidth :placement [H]
#+begin_src python :results file
fig, axes = plt.subplots(1, 2, figsize=(13, 5.5))
...
*Inference:* Ward linkage produces a visibly cleaner LAYING vs. STANDING
split than K-Means, matching its higher ARI/NMI in the Results table.

#+NAME: fig-confusion-matrices
#+CAPTION: Confusion matrices (majority-vote cluster to activity mapping)
...
#+NAME: fig-metrics-comparison
#+CAPTION: Internal and external evaluation metrics across algorithms
#+ATTR_LATEX: :width 0.9\linewidth :placement [H]
#+begin_src python :results file
fig, axes = plt.subplots(1, 2, figsize=(12, 4.5))
...
*Which algorithm produced the most meaningful clusters? Why?* By external
...
*How sensitive was K-Means to the choice of k?* Very sensitive. The
...
*Did DBSCAN detect noise or small clusters effectively?* Yes. The eps
...
*How does linkage choice (single/complete/ward) affect hierarchical
...
*Which internal metric best matched your visual intuition of cluster
...
(~0.14) corresponds to the overlapping SITTING/STANDING and
